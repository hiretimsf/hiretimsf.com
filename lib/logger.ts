...
class Logger {
  ...
  log(message: string, options?: LoggerOptions): void {
    ...
    console.log(this.prefix("log", options?.context), message, options?.meta || "");
  }

  info(message: string, options?: LoggerOptions): void {
    if (!this.shouldLog("info")) return;
    console.info(this.prefix("info", options?.context), message, options?.meta || "");
  }

  warn(message: string, options?: LoggerOptions): void {
    if (!this.shouldLog("warn")) return;
    console.warn(this.prefix("warn", options?.context), message, options?.meta || "");
  }
  ...
  error(message: string, error?: Error | unknown, options?: LoggerOptions): void {
    ...
    if (isDev) {
      console.error(this.prefix("error", options?.context), message, error, options?.meta || "");
    } else {
  ...
  debug(message: string, options?: LoggerOptions): void {
    ...
    console.debug(this.prefix("debug", options?.context), message, options?.meta || "");
  }
}
